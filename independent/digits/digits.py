...
# dataset had background color -1.0 and foreground color 1.0, so we need to flip
# the values and scale to -1.0, 1.0 range.
print("Testing additional images ...")
additional_images: Dict[int, torch.Tensor] = {}
for digit in range(10):
    pathname = f"/Users/dave/Temp/New images/{digit}.png"
    ...
    additional_images[digit] = image

for digit, image in additional_images.items():
    input_ = torch.unsqueeze(image, 0)
    with torch.no_grad():
        output = model(input_)
    probs = torch.exp(output)
    ...
    print(f"   Actual digit {digit}: predicted {predicted_digit} {'pass' if did_pass else 'FAIL'}")
