...
class SelfAttention(nn.Module):
    def __init__(self, config: Configuration):
        ...
        #   queries and keys are conceptually symmetrical, and together generate self-attention matrix
        ...
        # Example of last 2 dimensions for block size 4:
        #    1 0 0 0
        #    1 1 0 0
        #    1 1 1 0
        #    1 1 1 1
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        query_key_value = self.query_key_value_projection(x)    # batch_size x sequence_length x 3*embedding_size
        query, key, value = query_key_value.split(self.config.embedding_size, dim=2)

        # Split each of query, key, value into separate heads, and then transpose so that the two final dimensions
        # are sequence_length x head_size
